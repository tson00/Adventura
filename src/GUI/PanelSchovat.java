...
/**
 * Panel zobrazující list s věci, kde muzes se chovat
...
public class PanelSchovat implements Observer{
    ...
    */
    ...
    {        
        ...
        seznam = plan.getAktualniProstor().getVeci();
        data.clear();
        seznam.keySet().stream().map((x) -> seznam.get(x)).filter((pomocna) -> (pomocna.jePrenositelna()== false)).map((pomocna) -> new ImageView(new Image(main.Main.class.getResourceAsStream(pomocna.getObrazek()), 60, 60, false, false))).forEachOrdered((obrazek) -> {
            data.add(obrazek);
        });
    }
